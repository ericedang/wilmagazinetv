...
async function main() {
  ...
  // Google translate single API limits calls to ~100 per minute per IP or similar.
  // We can do batches, or just use the whole text. Wait, we can construct an array and translate them in chunks.
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // Google Translate supports multi-line up to 5k chars usually. 2000 to be safe.
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      chunk.keys.forEach((key, idx) => {
        esObj[key] = translatedLines[idx] || enObj[key]; // fallback if mismatched
      });
