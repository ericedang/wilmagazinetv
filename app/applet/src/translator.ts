...
async function main() {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      const translatedLines = (data[0] as any[]).map(item => item[0]).join('').split('\n');
      
      chunk.keys.forEach((key: string, idx: number) => {
        esObj[key] = translatedLines[idx] || enObj[key]; 
      });
      console.log(`Chunk ${i+1}/${chunks.length} translated`);
    } catch(e) {
      console.log("error on chunk", i);
      chunk.keys.forEach((key: string) => esObj[key] = enObj[key]);
    }
